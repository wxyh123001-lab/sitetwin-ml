"""
One-Class SVM hyperparameter search + final training.
Standalone: `python -m ml.train_ocsvm` trains directly from
training_data/normal_snapshots.pkl.

Search space: gamma (log-uniform). kernel is fixed to 'rbf' (the current
production default); nu is fixed to the configured contamination, same
reasoning as train_iforest.py -- contamination/nu is a stated assumption
about the data, shared across all three models, not a per-model tuning knob.
"""
import os
import time
...
def _build(trial, contamination):
    ...
    return OneClassSVM(nu=contamination, kernel="rbf", gamma=gamma)
...
def run(X_scaled, contamination, n_trials=25, n_splits=5, show_plots=True,
    ...
    final_model = OneClassSVM(nu=contamination, kernel="rbf", gamma=best_params["gamma"])
